<template>
    <div class="flex items-center justify-center min-h-screen">
        <UCard
            class="max-w-sm w-full mx-auto p-4 rounded-lg shadow-md"
        >
            <template #header>
                <b>发送站内信</b>
            </template>
            ...
            <UButtonGroup size="sm" orientation="horizontal">
                <UButton v-for="item in default_message" @click="message=item.content" color="white">{{ item.title }}</UButton>
            </UButtonGroup>
            <div style="margin: 10px;" />
...
async function sendMessage() {
    ...
    if (message.value === "" || receiver.value.split(',').map(Number).filter(Boolean).length === 0) {
